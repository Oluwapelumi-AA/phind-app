...
import { StyleSheet, Text, View, TouchableOpacity, Modal, ScrollView, Dimensions, TouchableWithoutFeedback, Image, Alert } from 'react-native';
...
const ResourcesDetails = () => {
  ...
  useEffect(() => {
    const fetchFeaturedImage = async () => {
      try {
        const response = await fetch(ResourcesData._links['wp:featuredmedia'][0].href);
        if (response.ok) {
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.educationWrapper]}>
              ...
              <View style={styles.innerDetailWrapper}>
                ...
                <View style={styles.iconWrapper}>
                  <View style={styles.iconAndImage}>
                    ...
                    <Text style={styles.iconText}>{ResourcesData.meta && ResourcesData.meta['application-email']}</Text>
                  </View>
                  <View style={styles.secondIconAndImage}>
                    <Icon style={styles.icon} name="phone" />
                    <Text style={styles.iconText}>{ResourcesData.meta && ResourcesData.meta['application-phone']}</Text>
                  </View>
                </View>
                <View style={styles.thirdIconAndImage}>
                  <Icon style={styles.icon} name="location-on" />
                  <Text style={styles.iconText}>{ResourcesData.meta.city}</Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.titleTextWrapper}>
              <Text style={styles.title}>{ResourcesData.title.rendered}</Text>
              <Text style={styles.subtext}>{removeTags(ResourcesData.content?.rendered)}</Text>
